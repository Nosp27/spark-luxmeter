...
async def client_for_app(request: aiohttp.web.Request):
    ...
    applications = await redis.zrevrangebyscore("applications")
...
async def client_for_app(request: aiohttp.web.Request):
    ...
    loader = application_loader.ApplicationLoader(
        app["METRICS_CLIENT"], app_id, fetch_last_jobs=5
    )
...
async def rm_client_for_app(request: aiohttp.web.Request):
    ...
    if app_id not in app["LOADERS"]:
        return aiohttp.web.json_response(
            {"error": "client does not exist"}, status=404
        )
...
def start():
    ...
    app["METRICS_CLIENT"] = clients.MetricsClient(active_jobs_only=False)
